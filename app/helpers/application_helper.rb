...
module ApplicationHelper
  ...
  @@topic_path_hide_controllers = ["article", "account"]
  ...
  def content_for_global_navi
    ...
    if controller.my_category == "article" && controller.action_name == "new"
      class_name = "activeNavi"
    else
      class_name = "nonActive"
    end

    ary << link_to(icon_tag("page_white_edit") + "新規メモ", {:controller => "article", :action => "new"}, {:class => class_name})

    if controller.my_category == "article" && (controller.action_name == "index" || controller.action_name == "list")
      class_name = "activeNavi"
    else
      class_name = "nonActive"
    end

    ary << link_to(icon_tag("book_open") + "メモ一覧", {:controller => "article", :action => "list"}, {:class => class_name})


    if controller.my_category == "article" && (controller.action_name == "graph")
      class_name = "activeNavi"
    else
      class_name = "nonActive"
    end

    ary << link_to(icon_tag("chart_line") + "グラフ", {:controller => "article", :action => "graph"}, {:class => class_name})




#     if development? || controller.local_request? || (session[:user] && session[:user].loginname == "ikeda")
#       if controller.my_category == "article" && (controller.action_name == "bookmark")
#         class_name = "activeNavi"
#       else
#         class_name = "nonActive"
#       end

#       ary << link_to(icon_tag("world") + "ブックマーク", {:controller => "article", :action => "bookmark"}, {:class => class_name})
#     end
  ...
  # タグクラウド
  # http://blog.craz8.com/article/2005/10/28/acts_as_taggable-is-a-cool-piece-of-code
  ...
  def tag_item_url(options)
    url_for(:controller => "article", :action => "search", :query => options[:query])
  end
  ...
  # For example:
  #
  #   <%= link_to "新規メモ", bookmarklet_javascript('article/bookmarklet') %>
  #
